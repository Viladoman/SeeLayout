...
//TODO ~ ramonv ~ input Home/End go to the top bottom of the list
//TODO ~ ramonv ~ double click for open in sources
//TODO ~ ramonv ~ context menus
...
var fieldVisualizer =
  [
    ...
    //Layout
    { key: 'size',                  display: 'Size',                   defaultWidth: 80,   type: DisplayType.None,    showTotal: false },
    { key: 'align',                 display: 'Align',                  defaultWidth: 80,   type: DisplayType.None,    showTotal: false },
    { key: 'real',                  display: 'Real Size',              defaultWidth: 80,   type: DisplayType.None,    showTotal: false },
    { key: 'pad',                   display: 'Padding Size',           defaultWidth: 80,   type: DisplayType.None,    showTotal: false },
    { key: 'padPCT',                display: 'Padding Percent',        defaultWidth: 100,  type: DisplayType.Percent, showTotal: false },
...
*/
  ...
  this.GetFieldContent = function(field,value)
  {
    if (field.visualizer)
    {
      switch(field.visualizer.type)
      {
        case DisplayType.None: return String(value);
        case DisplayType.Time: return Common.TimeToString(value);
        case DisplayType.Percent: return (Math.round(value * 100) / 100).toFixed(2) + ' %';
      }
    }
    else
    {
  ...
  this.RenderTotal = function(offset)
  {
    ...
    for(var i=0;i<_this.dataFields.length;++i)
    {
      ...
      if (field.visualizer != undefined && field.visualizer.showTotal != undefined && field.visualizer.showTotal)
      {
        ...
        var content = _this.GetFieldContent(field,_this.totalInfo[field.key]);
  ...
  this.RenderElement = function(element,offset)
  {
    ...
    for(var i=0;i<_this.dataFields.length;++i)
    {
      ...
      var content = _this.GetFieldContent(field,element[field.key]);
  ...
  this.Render = function()
  {
    ...
    {
      //TODO ~ ramonv ~ draw some nice empty letter saying no matches found
